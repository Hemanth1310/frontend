...
const Hero = ({searchQuery}) => {
   ...
   useEffect(()=>{
        ...
        try{
            ...
            const response = await fetch(
                `https://www.googleapis.com/books/v1/volumes?q=${encodeURIComponent(
                    searchQuery
                )}&maxResults=35&key=AIzaSyCppiMhGsciaLJuz9ZKDTytraEYAC16Z30`
            );
    ...
    if(loading){
        return(
            <div className='animate-pulse grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 p-6'>
                {[...Array(6)].map((_,i)=>(
                    <div key={i} className='h-64 md:h-96 bg-gray-800/50 rounded-xl md:rounded-2xl
    ...
    if(allDummy){
        return(
            <div className='min-h-screen bg-gradient-to-b fron-gray-900 to-gray-800 p-8 flex justify-center
            items-center'>
                <p className='tetx-cyan-300 text-lg md:text-xl text-center'>
                    No books found matching you search.
                </p>
            </div>
        )
    }
        ...
        <div className='max-w-7xl mx-auto'>
            <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 lg:gap-8'>
                {paginatedBooks.map((book,index)=>{
                    if(book.dummy){
                        return(
                            <div key={book.id || index} className='group relative bg-gray-800/30 rounded-xl md:rounded-2xl
                            backdrop-blur-xl border border-dashed border-gray-700/50 flex items-center justify-center
                            h-64 md:h-95'>
                    ...
                    return(
                        <div id='hero' key={book.id} className='group relative bg-gray-800/30 rounded-xl md:rounded-2xl
                        backdrop-blur-xl border border-white/10 hover:border-cyan-400/30 transition-all
                        duration-300 hover:-translate-y-1 md:hover:-translate-y-2 shadow-lg hover:shadow-xl md:shadow-2xl
                        hover:shadow-cyan-400/10'>
                            <a href={book.infoLink}
                                target='_blank' rel='nooperer noreferrer'
                                className='absolute inset-0 z-10 cursor-pointer'
                                aria-label={`View ${book.title}`}
                            />
                            <div className='p-4 md:p-6'>
                                <div className='relative aspect-[4/5] w-full rounded-xl md:rounded-2xl
                                overflow-hidde bg-gradient-to-br from-cyan-400/10 to-blue-400/10'>
                                    ...
                                    <div className='absolute bottom-0 left-0 right-0 p-2 md:p-4 bg-gradient-to-t
                                ...
                                <div className='mt-4 md:mt-6'>
                                        ...
                                        <div className='flex text-amber-400'>
                                            {[...Array(5)].map((_,i)=>(
                                                <StarIcon key={i} className={`h-4 w-4 md:h-5 md:w-5 
                                                    ${i<Math.floor(book.rating)?'fill-current':'fill-gray-600'}`}/>
                                            ))}
                                    ...
                                    <div className='mt-2 md:m-4 flex flex-wrap gap-1 ms:gap-2'>
                                        {book.categories.split(',').slice(0,3).map((category,index)=>(
                                            <span key={index} className='px-2 py-1 md:px-3 md:py-1 rounded-full
                                            bg-gray-700/50 text-xs text-cyan-300 backdrop-blur-sm'>
                                                {category.trim()}
                            ...
                            <div className='absolute inset-0 rounded-xl md:rounded-2xl bg-gradient-to-br from-cyan-400/20 to-blue-500/20
            ...
            <div className='mt-6 md:mt-8 flex flex-wrap justify-center items-center gap-2 md:gap-3'>
                ...
                className='px-3 py-1 md:px-4 md:py-2 rounded-md md:rpunded-lg bg-gray-900/80 border
                border-gray-700/50 text-cyan-300 text-xs md:text-sm hover:bg-gray-900/60 transition-colorsdisabled:opacity-50'>
                    Previous
                </button>
